...
public class AutoPortal extends Module {
    ...
    private BlockPos autoEnterPos;
    private int autoEnterTicks;
    ...
    private final Setting<Boolean> restoreSlot = sgGeneral.add(new BoolSetting.Builder()
        .name("restore-slot")
        .description("Restores your original hotbar slot after building and lighting the portal.")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Boolean> autoEnter = sgGeneral.add(new BoolSetting.Builder()
        .name("auto-enter")
        .description("Walks you into the lit portal after it is finished.")
        .defaultValue(false)
        .build()
    );

    private final Setting<Integer> autoEnterDelay = sgGeneral.add(new IntSetting.Builder()
        .name("auto-enter-delay")
        .description("Ticks to wait after lighting before auto-entering.")
        .defaultValue(10)
        .min(0)
        .sliderMax(60)
    ...
    public void onActivate() {
        startingSlot = mc.player != null ? mc.player.getInventory().getSelectedSlot() : -1;
        autoEnterPos = null;
        autoEnterTicks = -1;
        int obsidianCount = 0;
        for (int i = 0; i < 36; i++) {
    ...
    public void onDeactivate() {
        ...
        autoEnterPos = null;
        autoEnterTicks = -1;
        if (restoreSlot.get() && mc.player != null && startingSlot >= 0 && startingSlot <= 8) {
            mc.player.getInventory().setSelectedSlot(startingSlot);
        }
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (index < portalBlocks.size() && blockItem.getBlock().asItem() != Items.OBSIDIAN) return;

        if (autoEnterTicks >= 0) {
            if (autoEnterTicks-- <= 0 && autoEnterPos != null) {
                Vec3d target = Vec3d.ofCenter(autoEnterPos);
                Vec3d delta = target.subtract(mc.player.getEntityPos());
                if (delta.horizontalLengthSquared() > 0.09) {
        ...
        if (index >= portalBlocks.size()) {
            ...
            for (int i = 0; i < 9; i++) {
                if (mc.player.getInventory().getStack(i).getItem() == Items.FLINT_AND_STEEL) {
                    ...
                    mc.player.swingHand(Hand.MAIN_HAND);
                    autoEnterPos = firePos;
                    autoEnterTicks = autoEnter.get() ? autoEnterDelay.get() : -1;
                    break;
                }
            }
            if (autoEnter.get()) {
                info("Portal complete. Auto-entering.");
            } else {
