...
import static dev.forg.util.JeffUtils.*;
...
public class ElytraFlyPlusPlus extends Module {
    ...
    private final Setting<Boolean> antiRubberband = sgGeneral.add(new BoolSetting.Builder()
        ...
        .description("Slows the module temporarily after server movement corrections.")
        .defaultValue(true)
        .build()
    );

    private final Setting<Integer> correctionSlowTicks = sgGeneral.add(new IntSetting.Builder()
        .name("correction-slow-ticks")
        .description("Ticks to stay in reduced-speed mode after a server position correction.")
        .defaultValue(40)
        .min(0)
        .sliderMax(200)
    ...
    private final Setting<Double> correctionSpeedMultiplier = sgGeneral.add(new DoubleSetting.Builder()
        .name("correction-speed-multiplier")
        .description("Speed multiplier applied while recovering from a rubberband.")
        .defaultValue(0.65)
        .min(0.1)
        .max(1.0)
        .sliderMin(0.1)
    ...
    private int correctionRecoveryTicks = 0;
    ...
    private void onReceivePacket(PacketEvent.Receive event)
    {
        ...
        {
            if (antiRubberband.get()) correctionRecoveryTicks = correctionSlowTicks.get();
        }
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (lastPos != null)
        {
            ...
            if (antiRubberband.get() && correctionRecoveryTicks > 0) {
                targetSpeed *= correctionSpeedMultiplier.get();
            }

            if (mc.player.isOnGround() && mc.player.isSprinting() && speedBps < targetSpeed)
            {
    ...
    {
        if (mc.player == null || mc.player.getAbilities().allowFlying) return;
        if (correctionRecoveryTicks > 0) correctionRecoveryTicks--;
