...
public class Lavacast extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (dist < distMin.get()) { toggle(); return; }
        tick = 0;
    ...
    private void placeLava() {
        ...
        if (!findItemResult.found()) { error("No lava bucket found."); toggle(); return; }
    ...
    private void placeWater() {
        ...
        if (!findItemResult.found()) { error("No water bucket found."); toggle(); return; }
        int prevSlot = mc.player.getInventory().getSelectedSlot();
        mc.player.getInventory().setSelectedSlot(findItemResult.slot());
        mc.interactionManager.interactItem(mc.player, Hand.MAIN_HAND);
        mc.player.getInventory().setSelectedSlot(prevSlot);
    ...
    private void pickupLiquid() {
        ...
        if (!findItemResult.found()) { error("No bucket found."); toggle(); return; }
        int prevSlot = mc.player.getInventory().getSelectedSlot();
        mc.player.getInventory().setSelectedSlot(findItemResult.slot());
        mc.interactionManager.interactItem(mc.player, Hand.MAIN_HAND);
        mc.player.getInventory().setSelectedSlot(prevSlot);
    ...
    private void getDistance(Vec3i offset) {
        ...
        final BlockHitResult result = mc.world.raycast(new RaycastContext(
                Vec3d.ofCenter(pos), Vec3d.ofCenter(pos.down(250)),
