...
import static dev.forg.util.JeffUtils.positionInDirection;
...
public class TrailFollower extends Module
{
    ...
    public final Setting<Boolean> persistTrailState = sgGeneral.add(new BoolSetting.Builder()
        .name("persist-trail-state")
        .description("Saves the current trail state so you can resume after disconnects or restarts.")
        .defaultValue(true)
        .build()
    );

    public final Setting<Boolean> resumeSavedTrail = sgGeneral.add(new BoolSetting.Builder()
        .name("resume-saved-trail")
        .description("Loads the saved trail state on activation if one exists.")
        .defaultValue(true)
        .visible(persistTrailState::get)
        .build()
    );
    ...
    public void onActivate()
    {
        ...
        if (!OptionalModSupport.XAERO_PLUS_AVAILABLE) {
            error("TrailFollower requires XaeroPlus chunk data. Install XaeroPlus to use this module.");
            toggle();
            return;
        }
        ...
        if (persistTrailState.get() && resumeSavedTrail.get()) {
            loadTrailState();
            if (followingTrail) {
                info("Loaded saved trail state.");
            }
        }
    ...
    {
        if (persistTrailState.get()) saveTrailState();
        started = false;
    ...
    private Path trailStateFile() {
        return ForgPaths.dataDir().resolve("trail-follower-state.json");
    }

    private void saveTrailState() {
        try {
            Files.createDirectories(ForgPaths.dataDir());
            try (Writer writer = Files.newBufferedWriter(trailStateFile())) {
                TrailState state = new TrailState();
                state.followingTrail = followingTrail;
                state.targetYaw = targetYaw;
                state.lastFoundTrailTime = lastFoundTrailTime;
                state.lastFoundPossibleTrailTime = lastFoundPossibleTrailTime;
                state.followMode = followMode != null ? followMode.name() : null;
                state.trail = new ArrayList<>(trail);
                state.possibleTrail = new ArrayList<>(possibleTrail);
                GSON.toJson(state, writer);
            }
        } catch (IOException e) {
            error("Failed to save trail state: " + e.getMessage());
        }
    }

    private void loadTrailState() {
        if (!Files.exists(trailStateFile())) return;

        try (Reader reader = Files.newBufferedReader(trailStateFile())) {
            TrailState state = GSON.fromJson(reader, TrailState.class);
            if (state == null) return;

            followingTrail = state.followingTrail;
            targetYaw = state.targetYaw;
            lastFoundTrailTime = state.lastFoundTrailTime;
            lastFoundPossibleTrailTime = state.lastFoundPossibleTrailTime;
            trail.clear();
            possibleTrail.clear();
            if (state.trail != null) trail.addAll(state.trail);
            if (state.possibleTrail != null) possibleTrail.addAll(state.possibleTrail);
            if (state.followMode != null) {
                try {
                    followMode = FollowMode.valueOf(state.followMode);
                } catch (IllegalArgumentException ignored) {
                    followMode = null;
                }
            }
        } catch (IOException e) {
            error("Failed to load trail state: " + e.getMessage());
        }
    }
...
//                    }
            ...
            case YAWLOCK: {
                mc.player.setYaw(JeffUtils.smoothRotation(getActualYaw(mc.player.getYaw()), targetYaw, rotateScaling.get()));
                break;
            }
        }
...
//                log("ChunkPos: " + chunkPos.x + ", " + chunkPos.z);
        ...
        double angleDiff = JeffUtils.angleDifference(targetYaw, chunkAngle);
...
//    }
    ...
    private static class TrailState {
        ...
        private List<Vec3d> possibleTrail;
    }
}
