...
{
    private static DiscordNotifs INSTANCE;
    ...
    private final Setting<Boolean> includeContext = sgGeneral.add(new BoolSetting.Builder()
        .name("include-context")
        .description("Prefixes messages with server and dimension context.")
        .defaultValue(true)
        .build()
    );

    private final Setting<Boolean> moduleEvents = sgGeneral.add(new BoolSetting.Builder()
        .name("module-events")
        .description("Allows other Forg modules to send webhook events through this module.")
        .defaultValue(true)
        .build()
    );
    ...
    public void handleMessage(String message, MessageType messageType)
    {
        ...
        else if (moduleEvents.get() && messageType.equals(MessageType.MODULE))
        {
            sendWebhookMessage(message);
        }
    }
    ...
    private void sendWebhookMessage(String message)
    {
        ...
        if (includeContext.get())
        {
            message = "[" + getServerContext() + "][" + getDimensionContext() + "] " + message;
        }
        String json = "{\n" +
            "\"embeds\": [{" +
                "\"description\": \"" + message + "\"" +
    ...
    public static void pushModuleEvent(String source, String message)
    {
        if (INSTANCE == null || !INSTANCE.isActive() || !INSTANCE.moduleEvents.get()) return;
        INSTANCE.handleMessage("[" + source + "] " + message, MessageType.MODULE);
    }
    ...
    private String getDimensionContext()
    {
