package dev.forg.utils.oresim;

import dev.forg.mixin.accessor.CountPlacementModifierAccessor;
import dev.forg.mixin.accessor.HeightRangePlacementModifierAccessor;
import dev.forg.mixin.accessor.RarityFilterPlacementModifierAccessor;
...
public class Ore {
    ...
    public static Map<RegistryKey<Biome>, List<Ore>> getRegistry(Dimension dimension) {
        ...
        var dimensions = presets.getOrThrow(WorldPresets.DEFAULT).value().createDimensionsRegistryHolder().dimensions();
        ...
        List<PlacedFeatureIndexer.IndexedFeatures> indexer = PlacedFeatureIndexer.collectIndexedFeatures(
            biomeEntries, biomeEntry -> biomeEntry.value().getGenerationSettings().getFeatures(), true
        );
